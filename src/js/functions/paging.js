function addPageButtonListeners(dbh, name, columns, limit, order, classes, id) {
...
function redirectToPage(dbh, name, columns, limit, nextPage, order, classes, id) {
    ...
    queryArray.forEach(function(value, index, array) {
        if (queryStringNew == '') {
            queryStringNew += '?';
        } else {
            queryStringNew += '&';

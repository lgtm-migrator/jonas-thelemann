...
If ((-Not (Test-Path -Path $CaCrt)) -And (-Not (Test-Path -Path $CaKey))) {
    ...
    openssl req -config $CaCnf -keyout $CaKey -new -out $CaCrt -passout pass:$ClearPassword -x509
} Else {
...
openssl req -config $ProjectCnf -keyout $ProjectKey -new -nodes -out $ProjectCsr
